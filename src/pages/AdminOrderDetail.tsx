...
import {
  Box, Typography, CircularProgress, Divider, Select, MenuItem, Button,
} from '@mui/material';
import {
  ArrowLeft, ClipboardList, User, Package, CheckCircle2,
  Clock, Truck, XCircle, RefreshCw, CreditCard, Calendar, Hash,
} from 'lucide-react';
...
const InfoRow = ({ icon: Icon, label, value, valueColor }: { icon: any; label: string; value: string; valueColor?: string }) => (
  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, py: 1 }}>
    <Box sx={{ width: 32, height: 32, borderRadius: '8px', background: PINK[50], display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
      <Icon size={15} color={PINK[600]} />
    </Box>
    <Box>
      <Typography fontSize={11} color="text.disabled" lineHeight={1.2}>{label}</Typography>
      <Typography fontSize={14} fontWeight={700} sx={{ color: valueColor ?? '#1a1a1a' }}>{value}</Typography>
    </Box>
  </Box>
);
...
export const AdminOrderDetail = () => {
  ...
  if (!order) {
    return (
      <Box sx={{ textAlign: 'center', py: 10 }}>
        ...
        <Button onClick={() => navigate('/admin/orders')} sx={{ mt: 2, color: PINK[600] }}>Back to Orders</Button>
      </Box>
    );
  }
  ...
  return (
    <Box sx={{ background: '#f7f7fa', minHeight: '100vh' }}>
      {/* Top bar */}
      <Box sx={{ background: `linear-gradient(135deg, ${PINK[600]}, ${PINK[500]})`, px: { xs: 2, md: 4 }, py: 1.5, display: 'flex', alignItems: 'center', gap: 1 }}>
        <Box onClick={() => navigate('/admin/orders')} sx={{ display: 'flex', alignItems: 'center', gap: 0.8, color: 'rgba(255,255,255,0.85)', cursor: 'pointer', '&:hover': { color: '#fff' } }}>
          <ArrowLeft size={16} />
          <Typography fontSize={13} fontWeight={600}>All Orders</Typography>
        </Box>
        <Typography fontSize={13} sx={{ color: 'rgba(255,255,255,0.5)', mx: 0.5 }}>/</Typography>
        <Typography fontSize={13} fontWeight={700} color="#fff">#{order.id.slice(-8).toUpperCase()}</Typography>
      </Box>
      ...
      <Box sx={{ maxWidth: 1000, mx: 'auto', px: { xs: 1.5, sm: 2, md: 3 }, py: 3 }}>
        ...
        <Box sx={{ background: '#fff', borderRadius: '20px', overflow: 'hidden', boxShadow: '0 4px 20px rgba(0,0,0,0.07)', mb: 3 }}>
          <Box sx={{ background: `linear-gradient(135deg, ${PINK[600]}, ${PINK[500]})`, px: 3, py: 2.5, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 1.5 }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
              <ClipboardList size={22} color="#fff" />
              <Box>
                <Typography fontWeight={900} fontSize={18} color="#fff">Order #{order.id.slice(-8).toUpperCase()}</Typography>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.8, background: statusMeta.bg, color: statusMeta.color, px: 1.5, py: 0.8, borderRadius: '20px', fontWeight: 700, fontSize: 13 }}>
          ...
          <Box sx={{ p: { xs: 2, sm: 3 }, display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr', md: 'repeat(4,1fr)' }, gap: 0 }}>
            ...
            <InfoRow icon={Calendar} label="Date" value={new Date(order.createdAt).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })} />
            <InfoRow icon={CreditCard} label="Payment" value={order.ispayment ? 'Paid' : 'Pending'} valueColor={order.ispayment ? '#388E3C' : '#F57C00'} />
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr 340px' }, gap: 3, alignItems: 'flex-start' }}>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
            ...
            <Box sx={{ background: '#fff', borderRadius: '20px', overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
              <Box sx={{ px: 3, py: 2, borderBottom: '1px solid #f5f5f5', display: 'flex', alignItems: 'center', gap: 1 }}>
                ...
                <Typography fontWeight={800} fontSize={15}>Ordered Products</Typography>
              ...
              <Box sx={{ p: { xs: 1.5, sm: 2 }, display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                {order.products.length === 0 ? (
                  ...
                  <Box key={product.id || i} sx={{ display: 'flex', gap: 1.5, alignItems: 'center', p: 1.5, borderRadius: '12px', background: '#fafafa', border: '1px solid #f0f0f0' }}>
                    <Box
                      component="img"
                      src={product.image || 'https://placehold.co/60x60/f5f5f5/bbb?text=img'}
                      alt={product.title}
                      sx={{ width: 56, height: 56, borderRadius: '10px', objectFit: 'cover', flexShrink: 0, background: '#eee' }}
                    />
                    <Box sx={{ flex: 1, minWidth: 0 }}>
                      <Typography fontWeight={700} fontSize={14} noWrap>{product.title || 'Product'}</Typography>
                      {product.price !== product.offerprice && (
                        <Typography fontSize={12} sx={{ textDecoration: 'line-through', color: '#bbb' }}>₹{product.price}</Typography>
                      )}
                    </Box>
              ...
              <Box sx={{ p: { xs: 1.5, sm: 2.5 } }}>
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
                  ...
                  <Typography fontSize={14} fontWeight={600} color="success.main">FREE</Typography>
                </Box>
                <Divider sx={{ borderColor: '#f5f5f5', mb: 2 }} />
                <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                  <Typography fontWeight={800} fontSize={16}>Total</Typography>
                  <Typography fontWeight={900} fontSize={20} sx={{ color: PINK[600] }}>₹{order.totalprice.toFixed(0)}</Typography>
                </Box>
              </Box>
            </Box>
            ...
            <Box sx={{ background: '#fff', borderRadius: '20px', overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
              <Box sx={{ px: 3, py: 2, borderBottom: '1px solid #f5f5f5', display: 'flex', alignItems: 'center', gap: 1 }}>
                ...
                <Typography fontWeight={800} fontSize={15}>Customer Details</Typography>
              </Box>
              <Box sx={{ p: { xs: 2, sm: 3 } }}>
                {order.customer ? (
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                    <Box sx={{ width: 48, height: 48, borderRadius: '50%', background: `linear-gradient(135deg, ${PINK[600]}, ${PINK[500]})`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      <Typography fontWeight={800} color="#fff" fontSize={18}>
                        {order.customer.name?.[0]?.toUpperCase() ?? 'U'}
                      </Typography>
                    </Box>
                    <Box>
                      <Typography fontWeight={700} fontSize={15}>{order.customer.name}</Typography>
                      <Typography fontSize={13} color="text.secondary">{order.customer.email}</Typography>
                    </Box>
                  </Box>
                ) : (
                  <Typography fontSize={14} color="text.secondary">Customer details not available</Typography>
                )}
              </Box>
            </Box>
          </Box>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
            <Box sx={{ background: '#fff', borderRadius: '20px', overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,0.06)', position: { md: 'sticky' }, top: { md: 84 } }}>
              <Box sx={{ px: 3, py: 2, borderBottom: '1px solid #f5f5f5', display: 'flex', alignItems: 'center', gap: 1 }}>
                <RefreshCw size={17} color={PINK[600]} />
                <Typography fontWeight={800} fontSize={15}>Update Status</Typography>
              </Box>
              ...
              <Box sx={{ p: 2.5 }}>
                    ...
                    return (
                      <Box key={s} sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: i < arr.length - 1 ? 0 : 0 }}>
                        <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                          <Box sx={{ width: 32, height: 32, borderRadius: '50%', background: isDone ? m.bg : '#f5f5f5', border: `2px solid ${isDone ? m.color : '#e0e0e0'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'all 0.2s' }}>
                            <Icon size={14} color={isDone ? m.color : '#ccc'} strokeWidth={2} />
                          </Box>
                          {i < arr.length - 1 && (
                            <Box sx={{ width: 2, height: 20, background: isDone && currentIdx > thisIdx ? m.color : '#e0e0e0', my: 0.3, transition: 'background 0.2s' }} />
                          )}
                        </Box>
                ...
                <Typography fontSize={13} color="text.secondary" mb={1} fontWeight={600}>Change Status</Typography>
                <Select
                  fullWidth
                  ...
                  {ALL_STATUSES.map(s => (
                    <MenuItem key={s} value={s} sx={{ fontSize: 13 }}>
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        {(() => { const Icon = STATUS_META[s].icon; return <Icon size={13} color={STATUS_META[s].color} />; })()}
                        {STATUS_META[s].label}
                      </Box>
                    </MenuItem>
                  ))}
