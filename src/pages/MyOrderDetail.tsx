...
import {
  Box, Typography, CircularProgress, Divider, Button,
} from '@mui/material';
import {
  ArrowLeft, ClipboardList, Package, CheckCircle2,
  Clock, Truck, XCircle, RefreshCw, CreditCard, Calendar, Hash, MapPin,
} from 'lucide-react';
  ...
  if (!order) {
    return (
      <Box sx={{ textAlign: 'center', py: 10 }}>
        ...
        <Button onClick={() => navigate('/my-orders')} sx={{ mt: 2, color: PINK[600] }}>Back to Orders</Button>
      </Box>
    );
  }
  ...
  return (
    <Box sx={{ background: '#f7f7fa', minHeight: '100vh' }}>
      ...
      <Box sx={{ background: `linear-gradient(135deg, ${PINK[600]}, ${PINK[500]})`, px: { xs: 2, md: 4 }, py: 1.5, display: 'flex', alignItems: 'center', gap: 1 }}>
        <Box onClick={() => navigate('/my-orders')} sx={{ display: 'flex', alignItems: 'center', gap: 0.8, color: 'rgba(255,255,255,0.85)', cursor: 'pointer', '&:hover': { color: '#fff' } }}>
          <ArrowLeft size={16} />
          <Typography fontSize={13} fontWeight={600}>My Orders</Typography>
        </Box>
      ...
      <Box sx={{ maxWidth: 900, mx: 'auto', px: { xs: 1.5, sm: 2, md: 3 }, py: 3 }}>
        ...
        <Box sx={{ background: '#fff', borderRadius: '20px', overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,0.06)', mb: 3 }}>
          <Box sx={{ px: 3, py: 2, borderBottom: '1px solid #f5f5f5', display: 'flex', alignItems: 'center', gap: 1 }}>
            ...
            <Typography fontWeight={800} fontSize={15}>Order Tracking</Typography>
          </Box>

          {isCancelled ? (
            <Box sx={{ p: 3, display: 'flex', alignItems: 'center', gap: 2 }}>
              <Box sx={{ width: 48, height: 48, borderRadius: '50%', background: '#FFEBEE', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <XCircle size={24} color="#D32F2F" />
              </Box>
              <Box>
                <Typography fontWeight={800} fontSize={16} color="#D32F2F">Order Cancelled</Typography>
                <Typography fontSize={13} color="text.secondary">This order has been cancelled.</Typography>
              </Box>
            </Box>
          ) : (
            <Box sx={{ px: { xs: 2, sm: 3 }, py: 3 }}>
              ...
              <Box sx={{ display: { xs: 'none', sm: 'flex' }, alignItems: 'flex-start', justifyContent: 'space-between', position: 'relative' }}>
                ...
                <Box sx={{ position: 'absolute', top: 16, left: '10%', height: 3, zIndex: 1, transition: 'width 0.4s',
                ...
                {STATUS_FLOW.map((s, i) => {
                  ...
                  return (
                    <Box key={s} sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: 1, position: 'relative', zIndex: 2 }}>
                      ...
                      <Typography fontSize={11} fontWeight={done ? 700 : 400} mt={1} textAlign="center"
                        sx={{ color: done ? m.color : 'text.disabled', lineHeight: 1.3 }}>
                        {m.label}
                      </Typography>
                      {active && (
                        <Typography fontSize={10} color="text.disabled" textAlign="center" mt={0.3}>{m.desc}</Typography>
                      )}
                    </Box>
                  );
                })}
                  ...
                  return (
                    <Box key={s} sx={{ display: 'flex', alignItems: 'flex-start', gap: 2 }}>
                      ...
                      <Box sx={{ pt: 0.5, pb: i < STATUS_FLOW.length - 1 ? 0 : 0 }}>
                        ...
                        {active && (
                          <Typography fontSize={12} color="text.secondary">{m.desc}</Typography>
                        )}
                      </Box>
                    </Box>
                  );
        ...
        <Box sx={{ background: '#fff', borderRadius: '20px', overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,0.06)', mb: 3 }}>
          <Box sx={{ px: 3, py: 2, borderBottom: '1px solid #f5f5f5', display: 'flex', alignItems: 'center', gap: 1 }}>
            ...
            <Typography fontWeight={800} fontSize={15}>Ordered Items</Typography>
        ...
        <Button
          ...
          variant="outlined"
          sx={{ borderColor: PINK[500], color: PINK[600], borderRadius: '30px', px: 3, fontWeight: 700, '&:hover': { borderColor: PINK[600], background: PINK[50] } }}
        >
          Continue Shopping
        </Button>
      </Box>
