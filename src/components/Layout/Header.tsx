...
export const Header = () => {
  ...
  const handleLogout = () => { handleMenuClose(); signOut(); navigate('/login'); };
        ...
        >
          ...
          {searchOpen ? (
            ...
            >
              ...
              />
              <IconButton size="small" onClick={() => { setSearchOpen(false); setSearchVal(''); }} sx={{ p: 0.3 }}>
          ...
          <IconButton
            ...
            sx={{ color: C.grey, '&:hover': { color: C.pink }, display: { xs: 'none', sm: 'flex' } }}
          >
            <Heart size={20} />
          </IconButton>
          ...
          {isAuthenticated && user ? (
            <>
              <Avatar
                ...
                sx={{ width: 32, height: 32, fontSize: 12, fontWeight: 700, bgcolor: C.pink, color: '#fff', cursor: 'pointer', '&:hover': { opacity: 0.85 } }}
              >
                {initials}
              </Avatar>
              ...
              >
                ...
                <Divider />
                <MenuItem onClick={() => { handleMenuClose(); navigate('/profile'); }} sx={{ gap: 1.5, fontSize: 13, py: 1.2 }}>
                  <User size={14} color={C.pink} /> My Profile
                </MenuItem>
                <MenuItem onClick={() => { handleMenuClose(); navigate('/change-password'); }} sx={{ gap: 1.5, fontSize: 13, py: 1.2 }}>
                  <Lock size={14} color={C.pink} /> Change Password
                </MenuItem>
                <MenuItem onClick={() => { handleMenuClose(); navigate('/my-orders'); }} sx={{ gap: 1.5, fontSize: 13, py: 1.2 }}>
                  <ShoppingBag size={14} color={C.pink} /> My Orders
                </MenuItem>

                {user?.is_admin && <Divider />}
                {user?.is_admin && (
                  <MenuItem onClick={() => { handleMenuClose(); navigate('/admin/categories'); }} sx={{ gap: 1.5, fontSize: 13, py: 1.2, color: C.pink, fontWeight: 600 }}>
                    <Tag size={14} /> Manage Categories
                  </MenuItem>
                )}
                {user?.is_admin && (
                  <MenuItem onClick={() => { handleMenuClose(); navigate('/admin/products'); }} sx={{ gap: 1.5, fontSize: 13, py: 1.2, color: C.pink, fontWeight: 600 }}>
                    <Package size={14} /> Manage Products
                  </MenuItem>
                )}
                {user?.is_admin && (
                  <MenuItem onClick={() => { handleMenuClose(); navigate('/admin/orders'); }} sx={{ gap: 1.5, fontSize: 13, py: 1.2, color: C.pink, fontWeight: 600 }}>
